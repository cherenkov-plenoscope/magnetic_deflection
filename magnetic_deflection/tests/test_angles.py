...
def test_azimuth_range():
    assert 0 == sphcors._azimuth_range(0)
    assert 90 == sphcors._azimuth_range(90)
    assert 180 == sphcors._azimuth_range(180)
    assert -179 == sphcors._azimuth_range(181)
    assert -90 == sphcors._azimuth_range(-90)
    assert 180 == sphcors._azimuth_range(180)


def test_azimuth_range_array():
    arr = sphcors._azimuth_range(np.array([0.0, 90.0, 180.0, 181.0]))
    assert arr[0] == 0.0
    assert arr[1] == 90.0
    assert arr[2] == 180.0
    assert arr[3] == -179.0
...
def test_angles_scalars():
    for az_deg in np.linspace(-380, 380, 25):
        for zd_deg in np.linspace(0, 89, 25):
            ...
            az_deg_back, zd_deg_back = sphcors._cx_cy_to_az_zd_deg(
                cx=cx, cy=cy
            )

            dist_deg = sphcors._angle_between_az_zd_deg(
                az1_deg=az_deg,
                zd1_deg=zd_deg,
                az2_deg=az_deg_back,
                zd2_deg=zd_deg_back,
