...
parser = argparse.ArgumentParser(
    prog="plot_primary_deflection.py",
    description=("Show how deflection scales with energy"),
)
parser.add_argument(
    "--skymap_dir",
    metavar="PATH",
...
MAX_NUM_SHOWERS_TO_PLOT = int(5e3)
ALPHA = 0.1

KEYS_OF_INTEREST = [
    ("particle_cx", np.float32),
    ("particle_cy", np.float32),
    ("particle_energy_GeV", np.float32),
]
...
mask_function = mdfl.cherenkov_pool.reports.MaskCherenkovInCone(**POINTING)
...
if os.path.exists(cache_path):
    with open(cache_path, "rt") as f:
        showers = json_utils.loads(f.read())
else:
    reports = skymap.read_reports(
        dtype=KEYS_OF_INTEREST,
        mask_function=mask_function,
    )
    (
        showers["particle_azimuth_rad"],
        showers["particle_zenith_rad"],
    ) = spherical_coordinates.cx_cy_to_az_zd(
        cx=reports["particle_cx"],
        cy=reports["particle_cy"],
    )
    showers["particle_energy_GeV"] = reports["particle_energy_GeV"]

    with open(cache_path, "wt") as f:
        f.write(json_utils.dumps(showers))
...
FIELD_OF_VIEW["wide"]["particles"] = [pk]
