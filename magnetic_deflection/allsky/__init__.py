...
def plot_draw_particle_direction_with_masked_grid(result, debug, path):
    ...
    for i in range(len(hemisphere_grid.faces)):
        if i in debug["hemisphere_mask"]:
            mesh_look["faces_fill"][i] = svgplt.color.css("green")
    ...
    ax_add_marker(
        ...
        cx=[result_cx_cy[0]],
        cy=[result_cx_cy[1]],
        marker_half_angle_deg=2.0,
        marker_fill=[svgplt.color.css("red")],
        marker_fill_opacity=[0.75],
    )
    ...
    ax_add_marker(
        ...
        marker_fill=[
            svgplt.color.css("green") for i in range(len(rejected_cx_cy[0]))
        ],
        marker_fill_opacity=[0.75 for i in range(len(rejected_cx_cy[0]))],
    )
